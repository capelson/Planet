HTMLWidgets.widget({
  ...
  renderValue: function(el, x, instance) {
      ...
      // margin handling
      //   set our default margin to be 10
      ...
      var node = svg.selectAll(".node")
          ...
          .on("mouseover", null)
         /* .on("mouseover",function(d) {Shiny.onInputChange("node_piechart", d.name);})
      ...
      function dragmove(d) {
       ...
       function manualLayout() {
        const displacement_names = ['Net Exports', 'Other Production', 'Losses', 'Other Goods', 'Net Imports']
        const displacements_y = [20, 20, 20, 20, 20]
        var foo = d3.selectAll(".node")

        for (j=0; j < nodes.length; j++) {
          pickNode = foo["_groups"][0][j];
          d = nodes[j];

          if (displacement_names.includes(d.name)) {
            var node_name = d.name
            var position = displacement_names.indexOf(node_name)

            d3.select(pickNode).attr("transform",
                   "translate(" + (
                        d.x = d.x
                              ) + "," + (
                        d.y = d.y + displacements_y[position]
                              ) + ")");
          }
